...
import { getSent, countSent } from "@/services/EmailService"; 
...
export default function ListMail() {
  const [sent, setSent] = useState<SentInterface | null>(null);
  ...
  useEffect(() => {
    const performCheck = async () => {
      try {
        const newUnreadCount = await countSent();
    ...
    const intervalId = setInterval(performCheck, 5000); // Kiểm tra mỗi 5 giây
  ...
  if (!sent || sent.data.length === 0) {
    return <p className="p-4 text-gray-500">Không có mail nào trong hòm thư.</p>;
  }

  return (
    <div className="border-t border-gray-200">
      {sent.data.map((mail: MailData, index: number) => (
        <MailItem key={index} mail={mail} type='sent'/>
      ))}
    </div>
  );
}